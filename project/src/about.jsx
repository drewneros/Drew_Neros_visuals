    ...
    <section id="about" data-screen-label="02 About" style={{ padding: "96px 56px 56px" }}>
      ...
      <div style={{
        ...
        <div>
          ...
          <p style={{ fontSize: 18, lineHeight: 1.45, margin: "0 0 16px", maxWidth: "48ch" }}>I'm a photographer and retoucher based in Sydney, working between Australia, Europe and Asia. I make pictures with stillness in them, the kind you keep looking at after you've already scrolled past.



          </p>
          <p style={{ fontSize: 14, lineHeight: 1.6, color: "var(--fg-soft)", margin: "0 0 24px", maxWidth: "56ch" }}>Eight years on set — studios, apartments, streets figuring out the version of a person that only shows up after the third frame. I work small, fast, and honest. Clients trust me to deliver the look they imagined, and to know when to push past it.
          ...
          <p style={{ fontSize: 14, lineHeight: 1.6, color: "var(--fg-soft)", margin: "0 0 24px", maxWidth: "56ch" }}>
            With a background in modeling myself, I understand what agencies need
            before the brief is even written.
          </p>
...
// image is stored in localStorage so it survives reload. Hover to reveal a
// "change" / "remove" affordance once a portrait is set.
...
function PortraitSlot({ src }) {
  ...
  const acceptFile = (file) => {
    ...
    reader.onload = (e) => {
      ...
      setStored(url);
      try {localStorage.setItem(PORTRAIT_STORAGE_KEY, url);} catch (err) {/* quota */}
    };
  ...
  return (
    <div
      ...
      <>
          ...
          <style>{`
            [role="button"]:hover .portrait-hover{ opacity:1 }
          `}</style>
        </> :
